...
(() => {
  ...
  // Chỉ dùng những hàm cần thiết để dApp hoạt động
  const VIN_TOKEN_ABI = [
  ...
  const SWAP_CONTRACT_ABI = [
    // MON -> VIN (gửi MON native kèm value)
    "function swapMonForVin() payable",
    // VIN -> MON (cần approve trước)
  ...
  let lastBetAmountBN = null;     // lưu bet gần nhất cho nút Repeat/Half/Double
  let lastDiceGame = null;        // lưu kết quả gần nhất trong phiên này
  ...
  function initReadProvider() {
    ...
    // Load thông tin Dice sẵn (không cần ví)
    loadMinBet();
    refreshDicePool();
  }

  // ===== Load MIN_BET và bankroll của Dice =====
  ...
  async function connectWallet() {
    ...
    try {
      ...
      // Init write contracts với signer
  ...
  function onSwapAmountInput() {
    ...
    const toAmountEl = $("swapToAmount"); // nếu có
    if (!fromInput) return;
  ...
  async function onSwapAction() {
    ...
    try {
      if (swapDirection === "vin-to-mon") {
        ...
        // Kiểm tra allowance VIN cho hợp đồng Swap
  ...
  async function onDicePlay() {
    ...
    try {
      ...
      // Kiểm tra allowance VIN cho Dice
      const allowance = await vinWrite.allowance(currentAccount, DICE_CONTRACT_ADDRESS);
      if (allowance.lt(amountBN)) {
        if (statusEl) statusEl.textContent = "Sending approval transaction for VIN to Dice...";
      ...
      if (statusEl) statusEl.textContent = "Dice transaction confirmed. Updating result...";

      // Parse event Played từ logs
      ...
      if (
        ...
        if (statusEl) {
          statusEl.textContent =
            "Dice failed: BANK_NOT_ENOUGH (contract bankroll not enough VIN).";
        }
        alert(
          "Dice failed: BANK_NOT_ENOUGH – the Dice contract does not have enough VIN to pay prizes.\nPlease deposit more VIN into the Dice contract."
        );
      } else {
        if (statusEl) statusEl.textContent = "Dice game failed. See console for details.";
  ...
  function initApp() {
    ...
    // Nút "Swap VIN/MON" & "Play Dice Now" trên trang Home
